...
router.get('/antal/:antal', async (req, res) => {
    ...
    try {
        ...
        const produkter = await Produkt.find({}, 'titel teaser image pris oprettet kommentar likes').sort([['oprettet', -1]]).limit(antal);
        res.json(produkter);
...
router.get('/soeg/:soegeord', async (req, res) => { //
    ...
    try {
        ...
        }, 'titel teaser image pris kommentar likes')
